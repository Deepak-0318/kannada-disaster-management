    ...
    with _memory_lock:
        ...
        if session_id not in _sessions:
            ...
            try:
                ...
                analytics_service.record_session()
            except Exception as e:
...
def ask_chatbot(
    ...
    # Expand retrieval query if it's a short follow-up query to retain context
    retrieval_query = query
    if turns and len(query.strip()) < 30:
        last_user_query = next((t["content"] for t in reversed(turns) if t["role"] == "user"), "")
        if last_user_query:
            retrieval_query = f"{last_user_query} {query}"
            logger.info(f"RAG query expanded with session memory context: '{retrieval_query}'")

    context, retrieval_confidence, citations = retrieve_context(retrieval_query)
